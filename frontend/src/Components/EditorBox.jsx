        ...
        try {
            ...
            if (error.response) {
                if (error.response.status === 401 || error.response.status === 403) {
                    ...
                    setVerdict(error.response.data.error || "Error in submission");
    ...
    return (
        <div className="">
            <div className="w-full min-h-12 pl-4 pt-2 flex items-end gap-4">
                ...
                ${activeTab === 'output' ? 'bg-gradient-to-br text-xl  from-[#ffb347] via-[#ddd28f] to-[#6a82fb] tracking-wider text-red-600' : 'bg-cyan-500 hover:bg-blue-500 text-white hover:text-yellow-500'}`}
                    onClick={() => setActiveTab('output')}>
                    Output
                </button>
                <button className={`truncate w-1/4 p-2 rounded-l-md h-fit font-newsreader rounded-br-xl rounded-tr-4xl transition-all duration-500
                ${activeTab === 'verdict' ? 'bg-gradient-to-br text-xl from-[#ffb347] via-[#ddd28f] to-[#6a82fb] tracking-wider text-red-600' : 'bg-cyan-500 hover:bg-blue-500 text-white hover:text-yellow-500'}`}
            ...
            <div className="flow-shadow flex flex-col items-center min-h-screen w-full rounded-xl bg-gradient-to-br from-[#e6b93e] via-[#bebcb0] to-[#e6b93e]">
                ...
                <textarea className="w-[600px] bg-[#282a36] text-amber-50 mt-4 p-2 border rounded"
                    rows={3}
                ...
                {activeTab === "verdict" && (
                    <>
                        ...
                        <div className="w-[600px] h-fit bg-amber-100 mb-10 p-5 font-tomorrow rounded-br-xl rounded-tr-4xl">
                            ...
                            <p className="overflow-x-auto text-md p-4 rounded-md mt-2 bg-[#282a36] text-amber-100">
